#!/usr/bin/env bash
# Creates API Gateway routes for the redaction logs endpoints, mirroring the
# existing /redact wiring (AWS_PROXY to the redaction lambda + MOCK CORS).
...
LAMBDA=moderate-api-redaction-lambda
...
create_get_route() {
  local parent_id="$1" path_part="$2" full_path="$3" perm_sid="$4"
  ...
  # GET -> AWS_PROXY (auth handled inside the lambda)
  aws apigateway put-method --rest-api-id "$API" --resource-id "$rid" \
    --http-method GET --authorization-type NONE --no-api-key-required >/dev/null
  ...
  aws apigateway put-integration-response --rest-api-id "$API" --resource-id "$rid" \
    --http-method OPTIONS --status-code 200 \
    --response-parameters "{\"method.response.header.Access-Control-Allow-Headers\":\"${CORS_HEADERS}\",\"method.response.header.Access-Control-Allow-Methods\":\"'OPTIONS,GET'\",\"method.response.header.Access-Control-Allow-Origin\":\"'*'\"}" >/dev/null

  # Allow API Gateway to invoke the lambda for this GET route.
  aws lambda add-permission --function-name "$LAMBDA" --statement-id "$perm_sid" \
    --action lambda:InvokeFunction --principal apigateway.amazonaws.com \
    --source-arn "arn:aws:execute-api:${REGION}:${ACCOUNT}:${API}/*/GET/${full_path#/}" >/dev/null 2>&1 \
    && echo "added invoke permission ${perm_sid}" || echo "permission ${perm_sid} already exists (skipped)"

  echo "wired GET ${full_path}"
}

create_get_route "$ROOT_ID" "redaction-logs" "/redaction-logs" "apigateway-redaction-logs-dev"
create_get_route "$DASHBOARD_ID" "redaction-logs" "/dashboard/redaction-logs" "apigateway-dashboard-redaction-logs-dev"
...
aws apigateway create-deployment --rest-api-id "$API" --stage-name "$STAGE" \
  --description "Add redaction logs routes" --query "id" --output text \
